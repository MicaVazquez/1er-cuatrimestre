...
int parser_EntidadFromText(FILE* pFile , LinkedList* pArrayList)
{
  ...
  fscanf(pFile,"%[^,],%[^,],%[^\n]\n",id,nombre,horasTrabajadas);//falsa lectura

   while(feof(pFile)==0)
   {
	     cant = fscanf(pFile,"%[^,],%[^,],%[^\n]\n",id,nombre,horasTrabajadas);

	     if(cant == 3)//varia segun cantidad de datos del archivo IMPORTANTE
	     {
           unEntidad = eEntidad_newParametros(id,nombre,horasTrabajadas);
