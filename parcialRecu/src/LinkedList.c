...
int laQuefiltra1(void* unEntidad)
{
    int idDificultad;
	int retorno = 0;
...
int laQuefiltra2(void* unEntidad)
{
	int idDificultad;
	int retorno = 0;
...
int laQuefiltra3(void* unEntidad)
{
	int idDificultad;
	int retorno = 0;
...
int laQuefiltra(void* unEntidad)
{
	...
	if(edad>30 && stricmp(profesion,"programador")==0)
	{
		retorno = 1;
	}
...
int ll_map(LinkedList* lista,int (*pFunc)(void*))
{
    if(lista!=NULL)
    {
        ...
        for(i=0; i < len; i++)
        {
            eEntidad* aux = (eEntidad*)ll_get(lista, i);//recorre todos los empleados
...
int laQueMapea(void* unEntidad)
{
	...
	eEntidad_getPrecio(unEntidad, &auxPrecio);

	if(editorialId == 1)
	{
        //20
		newPrecio = auxPrecio-(auxPrecio*20/100);
		eEntidad_setPrecio(unEntidad,newPrecio);
	}
...
int laQueMapea(void* unEntidad)
{
	...
	eEntidad_getHoras(unEntidad, &horas);

	if(horas >= 0 && horas <= 200)
	{

      eEntidad_setDedicacion(unEntidad,"estandar");
	}
	else if(horas<=300)
	{
		eEntidad_setDedicacion(unEntidad,"sobre la media");
	}
	else
	{
		eEntidad_setDedicacion(unEntidad,"excesivo");
	}
	return 1;
}
